    ...
    try {
        ...
        for await (const message of response) {
            ...
            if (message.type === 'result') {
                ...
                // Send ready event, unless subagents are still running. When it
                // is deferred, releaseDeferredReady() sends it later and clears
                // providerAuthFailed there — so the expired-login signal is not
                // dropped just because a subagent outlived the turn.
                if (!readyDeferred) {
                    if (providerAuthFailed) {
