...
import { backgroundWorkCount, getRigActivityIndicators, getRigComposerState, getRigGitSummary, getRigIdentity, hasBackgroundActivity, isRigMetadata, isRigMetadataV1, rigSendsMessageReceipts } from './rig';
...
function buildSessionRowData(
    ...
    return {
        ...
        // unread when there are messages past the read point. Absent lastReadSeq
        // means "read" (no unread) — matches the old restart-clears-unread
        // behavior and avoids flagging all history on first run. The session
        // you're currently viewing never shows unread (its read point catches up
        // when you leave).
...
interface StorageState {
    ...
    clearFeed: () => void;
    // Which session the user is currently viewing (memory-only) — excluded from
    // the unread indicator. Unread itself is derived from synced read position.
    currentViewingSessionId: string | null;
    setCurrentViewingSession: (sessionId: string | null) => void;
    /**
     * Chats the user has archived, before any machine has agreed to it.
     *
     * Memory-only: an app that was restarted has no archive still in flight,
     * and the server has long since answered.
     */
...
function buildSessionListViewData(
    sessions: Record<string, Session>,
    currentViewingSessionId: string | null,
    // Required on purpose: rows grey out on their machine's presence, and an
    // omitted map would quietly report every machine as online.
    machines: Record<string, Machine>,
    ...
    // Chat lists always sort by last activity — upstream stopped reading the
    // legacy toggle. The key is turn completion, NOT updatedAt: updatedAt bumps
    // on every background agent update and on metadata writes (read-position,
    // star, mode), either of which would make the list jump. See
    // sessionActivitySortKey.
    const sortKey = sessionActivitySortKey;
    ...
    const toRow = (session: Session) => buildSessionRowData(session, archivingSessionIds, currentViewingSessionId, machines, projects);
...
export const storage = create<StorageState>()((set, get) => {
    ...
    return {
        ...
        applySessions: (sessions: (Omit<Session, 'presence'> & { presence?: "online" | number })[]) => {
            ...
            // Canonical chat-list order: newest activity first, keyed on turn
            // completion rather than updatedAt (see sessionActivitySortKey).
            const sortKey = sessionActivitySortKey;
            ...
            // Unread is now derived per-row from metadata.lastReadSeq vs
            // lastMessageSeq (read-position, synced cross-client) — no local set.
        ...
        updateSessionDraft: (sessionId: string, draft: string | null) => set((state) => {
            ...
            return {
                ...state,
                sessions: updatedSessions,
                sessionListViewData: buildSessionListViewData(updatedSessions, state.currentViewingSessionId, state.machines, state.archivingSessionIds, state.projects)
            };
        }),
        ...
        deleteMachine: (machineId: string) => set((state) => {
            ...
            return {
                ...state,
                machines: remaining,
                sessionListViewData: buildSessionListViewData(state.sessions, state.currentViewingSessionId, remaining, state.archivingSessionIds, state.projects)
            };
        }),
        // Artifact methods
        ...
        deleteSession: (sessionId: string) => set((state) => {
            ...
            const sessionListViewData = buildSessionListViewData(remainingSessions, state.currentViewingSessionId, state.machines, state.archivingSessionIds, state.projects);
        ...
        setCurrentViewingSession: (sessionId: string | null) => set((state) => {
            ...
            // The viewing session is excluded from the unread indicator, so both
            // the old and new session's row can change — rebuild. The read-position
            // write (metadata.lastReadSeq) is done by the caller via ops.
            return {
                ...state,
                currentViewingSessionId: sessionId,
                sessionListViewData: buildSessionListViewData(state.sessions, sessionId, state.machines, state.archivingSessionIds, state.projects),
            };
        }),
        applySessionLastMessage: (sessionId: string, seq: number, createdAt?: number) => set((state) => {
            ...
            return {
                ...state,
                sessions,
                sessionListViewData: buildSessionListViewData(sessions, state.currentViewingSessionId, state.machines, state.archivingSessionIds, state.projects),
            };
        }),
        setSessionLastReadSeqOptimistic: (sessionId: string, seq: number) => set((state) => {
            const session = state.sessions[sessionId];
            if (!session?.metadata) return state;
            const metadata = { ...session.metadata, lastReadSeq: seq };
            const sessions = { ...state.sessions, [sessionId]: { ...session, metadata } };
            return {
                ...state,
                sessions,
                sessionListViewData: buildSessionListViewData(sessions, state.currentViewingSessionId, state.machines, state.archivingSessionIds, state.projects),
        ...
        markArchiving: (sessionId: string) => set((state) => {
            ...
            return {
                ...state,
                archivingSessionIds,
                sessionListViewData: buildSessionListViewData(state.sessions, state.currentViewingSessionId, state.machines, archivingSessionIds, state.projects),
            };
        }),
