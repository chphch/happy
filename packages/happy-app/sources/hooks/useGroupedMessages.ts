    ...
    for (const [turn, indexes] of segments) {
        ...
        const flushRun = () => {
            ...
            groups.push({
                ...
                item: {
                    type: 'agent-work-group',
                    // Keyed by the message the run completes at, never by its
                    // oldest member: history pages arrive oldest-last, so an
                    // oldest-member key shifts as a turn's older work loads.
                    // The newest run completes at the turn's final answer, which
                    // is also the key upstream settled on for an unsplit turn;
                    // every older run completes at the selection element that
                    // split it off, so the runs of one turn stay distinct.
                    id: `work-${boundaryId}`,
