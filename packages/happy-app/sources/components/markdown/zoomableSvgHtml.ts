/**
 * The zoomable-SVG page both fullscreen viewers run inside a WebView.
 *
 * Zooming is left to the browser on purpose. Chromium keeps the vector and
 * re-rasterises it in tiles as the zoom changes, so detail keeps arriving all
 * the way in at constant memory. Drawing the same picture with a native SVG
 * view instead means one offscreen bitmap whose bytes grow with the SQUARE of
 * the zoom — measured: baking 4x of a 1080x2340 screen asks for a 143 MB
 * bitmap and Android answers with "Canvas: trying to draw too large", killing
 * the app. That is the whole reason this page exists rather than a
 * react-native-svg viewer.
 *
 * Two properties of the script below are load-bearing and were each paid for
 * once (see MermaidViewer's history):
 *   - No RESTING `will-change: transform`. It pins the layer's raster scale, so
 *     every zoom level becomes an upscaled 1x bitmap. It is switched on only
 *     for the duration of a gesture.
 *   - COMMIT on settle. When the fingers lift, the zoom is baked into the
 *     element's own width/height and the transform returns to 1, which is what
 *     lets the compositor rasterise at the new size.
 */
export type ZoomableSvgHtmlOptions = {
    /** Extra tags for <head> — e.g. a renderer script. May be empty. */
    head: string;
    /** JS that fills `#stage`. Runs inside a try/catch that shows the error. */
    populate: string;
};

export function zoomableSvgHtml(opts: ZoomableSvgHtmlOptions): string {
    return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
...
<style>
  ...
  #stage svg,#stage img{display:block;max-width:100vw;max-height:100vh;height:auto;}
...
${opts.populate}
  ...
  function svgEl(){ return stage.querySelector('svg,img'); }
