...
const SessionItem = React.memo(({ session, selected, isFirst, isLast, isSingle }: {
    ...
    // With the experimental layout on there is no 'unread' status text — unread
    // is carried by the bold title instead, so the status line keeps reporting
    // what the session is actually doing. Attention states outrank both.
    const statusText = session.state === 'input_required'
        ? t('status.inputRequired')
        : session.state === 'permission_required'
            ? t('status.permissionRequired')
            : (!expUnreadBoldTitle && session.hasUnread)
                ? t('status.unread')
