...
export const CompactSessionRow = React.memo(({ session, selected, showBorder }: { session: SessionRowData; selected?: boolean; showBorder?: boolean }) => {
    ...
    // A session the agent is waiting on owns the dot either way — that is
    // attention, not unread — so it keeps the live colour and pulse.
    ...
    const status = (!expUnreadBoldTitle && session.hasUnread && !needsUserAction)
    ...
    const renderTrailingIndicator = () => {
        ...
        } else if (!expUnreadBoldTitle && session.hasUnread) {
