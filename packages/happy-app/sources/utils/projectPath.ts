...
/**
 * Orders project groups within a machine: starred first, then alphabetically by
 * display path. An empty `starred` set (the feature switched off) collapses this
 * to plain alphabetical order.
 */
export function compareProjectsByStar(
    a: { path: string; displayPath: string },
    b: { path: string; displayPath: string },
    machineId: string,
    starred: ReadonlySet<string>,
): number {
    if (starred.size > 0) {
        const starredA = starred.has(projectStarKey(machineId, a.path));
        const starredB = starred.has(projectStarKey(machineId, b.path));
